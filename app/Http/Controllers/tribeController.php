...
class TribeController extends CallApiController
{
	...
	$post_data='{"agentname":"'.$data['agent_name'].'",
	    ...
	    public function UploadBankStatement(Request $req){
	    	...
	    	if(isset($req->transaction_id) && $req->transaction_id){
				$post_data='{"secret":"'.TribeController::$secret.'","document_password":'.$pdf_pwd.',"loan_application_id":'.$loan_id.',"statement_file":"data:application/pdf;base64,'.$base64.'","transaction_id": "'.$req['transaction_id'].'"}';
	...
	public function CloseTransaction(Request $req){
	    ...
	    if(($data->error!=1)){
	    	 return Response::json(array(
	     					'status'=>true,
                            'document_url' => $data->response->document_url,
