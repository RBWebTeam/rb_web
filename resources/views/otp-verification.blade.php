...
<div class="container">
        <div class="row">
            ...
            <img src="images/verification_img.png" style="margin:0 auto; display:block; padding:20px;"/>
                        ...
                        <form name="express_otp_form" id="express_otp_form" method="POST" style="padding:0px 30px 20px 30px;">
                         {{ csrf_field() }}
                            ...
                            <div class="form-group">
                                ...
                                <input id="express_mobile" name="mobile" type="text" placeholder="+91 XXXXX XXXXX" class="form-control input-md" onkeypress="return fnAllowNumeric(event)" minlength="10" maxlength="10" required="">
                            </div>
                            
                            
                            <!-- Button -->
                            <div class="form-group">
                                <a class="btn btn-default btn-sm" id="express_get_otp" style="margin:0 auto; display:block;">GET OTP</a>
                            </div>
                        </form>
                    </div>
                                </div>
            ...
            <img src="images/verification_img.png" style="margin:0 auto; display:block; padding:20px;"/>
                        <!-- <p>Easy to apply for a loan with us,Once you have complete this form. </p> -->
                        <form name="express_verify_form" id="express_verify_form" method="POST" style="padding:0px 30px 20px 30px;">
                         {{ csrf_field() }}
                            ...
                            <!-- Button -->
                            <div class="form-group">
                                <a class="btn btn-default btn-sm" id="express_verify" style="margin:0 auto; display:block;">VERIFY OTP</a>
...
<script type="text/javascript">
    $('#express_get_otp').click(function(event){
      event.preventDefault();
      // alert('okae');
      if (! $('#express_otp_form').valid()) {

      } 
      else {
      if(($('#express_mobile').val().length)<10)
        {
              $('#mobile_value').show();
              return false;
            }
      ...
      $.ajax({  
         type: "POST",  
         url: "{{URL::to('express-send-otp')}}",
         data : $('#express_otp_form').serialize(),
    ...
    $("#express_verify").click(function(event){
    event.preventDefault();
     
      if (! $('#express_verify_form').valid()) {
    ...
    $.ajax({  
     type: "POST",  
     url: "{{URL::to('express-verify')}}",
     dataType:"json",
     data : $('#express_verify_form').serialize(),
     success: function(msg){
       ...
       if(msg.data=="true"){
        ...
        window.location.href="{{Session::get('url_session')}}";
