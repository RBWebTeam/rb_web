<?php echo $__env->make('layout.header', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
<div class="container">
        <div class="row img-center">
            <div class="col-md-12">
            
                <div class="wrapper-content bg-white pinside40">
                 <h1 class=""><center>Express Loan</center></h1>
                 </br>
                    <div class="row">
                   
                    <!-- Business Loan -->
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                            <h3 style="background:#ed1c24;color:#fff; font-size:14px; padding:10px; text-align: center;">Business Loan</h3>
                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a><img src="images/express-loan/tata-capital.png" alt="Tata Capital BL"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a class="title">Tata Capital BL</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('tata-capital-business-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/lendingkart.png" alt="LendingKart"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a class="title">LendingKart</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('lendingkart')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/aditya-birla-group.png" alt="Aditya Birla Group"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a class="title">Aditya Birla</a></h4>
                                    
                                    <a onclick="Redirect();" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                </div>
                            </div>

                             
                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                
                                <div class="card-img">
                                    <a href="tribe.html"><img src="images/express-loan/tribe.png" alt="tribe"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="tribe.html" class="title">Tribe</a></h4>
                                    
                                    <a href="tribe.html" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->
                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/edelweiss.png" alt="Edelweiss"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a class="title">Edelweiss</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('edelweiss')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->

                             <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/hdfc-bank.png" alt="HDFC"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a class="title">HDFC Business Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('hdfc-business-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/CapitalFloat.png" alt="Capital First"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a class="title">Capital First</a></h4>
                                    
                                    <!-- <a href="<?php echo e(URL::to('coming-soon')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a> -->
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/capital_first.png" alt="Capital First"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a class="title">Capital Float</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('coming-soon')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                        </div>
                        <!-- Business Loan end -->

                        <!-- Personal Loan -->
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                            <h3 style="background:#ed1c24;color:#fff; font-size:14px; padding:10px; text-align: center;">Personal Loan</h3>

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="rbl-personal-loan.html"><img src="images/express-loan/rbl-bank.png" alt="RBL Personal Loan"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="rbl-personal-loan.html" class="title">RBL Personal Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('rbl-personal-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="<?php echo e(URL::to('kotak-personal-loan')); ?>"><img src="images/express-loan/kotak-mahindra-bank.png" alt="Kotak Home Loan"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('kotak-personal-loan')); ?>" class="title">Kotak Personal Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('kotak-personal-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="iifl-loan-apply.html"><img src="images/express-loan/iifl.png" alt="IIFL"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="iifl-loan-apply.html" class="title">IIFL</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('apply-iifl-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a><img src="images/express-loan/tata-capital.png" alt="Tata Capital PL"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a class="title">Tata Capital PL</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('contact-us')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->

                             <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/capital_first.png" alt="Capital First"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a class="title">Capital First</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('coming-soon')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a ><img src="images/express-loan/hdfc-bank.png" alt="HDFC"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a class="title">HDFC Personal Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('hdfc-personal-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->
                        </div>
                        <!-- Personal Loan end -->
                        <!-- home loan -->
                        <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <h3 style="background:#ed1c24;color:#fff; font-size:14px; padding:10px; text-align: center;">Home Loan</h3>
                        <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="<?php echo e(URL::to('kotak-home-loan')); ?>"><img src="images/express-loan/kotak-mahindra-bank.png" alt="Kotak Home Loan"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('kotak-home-loan')); ?>" class="title">Kotak Home Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('kotak-home-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="<?php echo e(URL::to('yesbank-home-loan')); ?>"><img src="images/express-loan/yes-bank.png" alt="Yes Bank"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('yesbank-home-loan')); ?>" class="title">Yes Bank</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('yesbank-home-loan')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="contact-us.html"><img src="images/express-loan/hdfc-bank.png" alt="HDFC"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('contact-us')); ?>" class="title">HDFC Bank Home Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('contact-us')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->

                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="contact-us.html"><img src="images/express-loan/vastu.png" alt="HDFC"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('contact-us')); ?>" class="title">Vastu Home Loan</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('contact-us')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->
                    </div>
                    <!-- home loan end -->
                    <!-- short term loan -->
                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <h3 style="background:#ed1c24;color:#fff; font-size:14px; padding:10px; text-align: center;">Short Term Personal Loan</h3>
                        <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="early-salary.html"><img src="images/express-loan/early-salary.png" alt="Early Salary"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('early-salary')); ?>" class="title">Short Term PL</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('early-salary')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->
                            <!-- pay sence -->
                            <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="early-salary.html"><img src="images/express-loan/pay.png" alt="Early Salary"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('contact-us')); ?>" class="title">Short Term PL</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('contact-us')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                    
                                </div>
                            </div>
                            <!-- /.card listing -->
                    </div>
                    <!-- sort term loan end -->
                    <!-- POS loan -->
                     <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                        <h3 style="background:#ed1c24;color:#fff; font-size:14px; padding:10px; text-align: center;">POS Loan</h3>
                         <div class="card-listing how-it-block2 hvr-float-shadow2">
                                <!-- card listing -->
                                <div class="card-img">
                                    <a href="iifl-loan-apply.html"><img src="images/express-loan/iifl.png" alt="IIFL"></a>
                                </div>
                                <div class="card-content" style="text-align: center;">
                                    <h6 class="text-center text-danger"><i>Coming Soon</i></h6>
                                    <h4 class="card-name"><a href="<?php echo e(URL::to('contact-us')); ?>" class="title">IIFL</a></h4>
                                    
                                    <a href="<?php echo e(URL::to('contact-us')); ?>" class="btn btn-default btn-sm" style="margin-right: 0px;">Apply Now</a>
                                   
                                </div>
                            </div>
                            <!-- /.card listing -->
                    </div>


                    </div>
                </div>
            </div>
        </div>
    </div>

<?php echo $__env->make('layout.footer', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
<?php echo $__env->make('layout.script', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>

<script type="text/javascript">
    function Redirect() 
          {
            
            window.location="http://www.abfldirect.com?utm_source=RUPEEBOSS&utm_medium=DSA&utm_campaign=DirectSME";
            
          }
</script>

                   

                     
                            
                            
                       

